...
function SubCategoryProducts() {
  ...
  //   // Get back name in its real form
  const mainCategoryConverted = mainCategory?.replace(/-/g, " ");
  const categoryConverted = category?.replace(/-/g, " ");
  const subCategoryConverted = subCategory?.replace(/-/g, " ");
  ...
  const fetchProducts = async () => {
    try {
      ...
      const response = await axiosBackend.get(
        `/api/v1/product?subCategory=${subCategoryConverted}&limit=${limit}&page=${currentPage}`
      );
